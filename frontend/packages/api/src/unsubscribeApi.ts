import type { UnsubscribeRequest, UnsubscribeResult, UnsubscribePreview } from '@emailibrium/types';
...
export async function undoUnsubscribe(batchId: string): Promise<{ restored: number }> {
  return api.post(`unsubscribe/undo/${batchId}`).json<{ restored: number }>();
}

export async function previewUnsubscribe(subscriptionIds: string[]): Promise<UnsubscribePreview[]> {
  return api
