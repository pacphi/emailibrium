...
Mixing bare and numbered placeholders in the _same_ query string is not attempted by any call site in this codebase today and is not a case this translator is designed to make sensible — each query picks one style, matching existing convention.
...
The `.bind()` chain is written once per match arm (sqlx's builder type is concretely `Sqlite`- or `Postgres`-flavored once bound to a pool, so it cannot be shared across both in a single expression without a generic function) — but the _query text_ and the _business logic_ are authored exactly once. This is the same "duplicate only the unavoidable, share everything else" shape ADR-033's migration-directory split already uses.
