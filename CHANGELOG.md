...
- Eliminate all dead code and implement all Phase A/C stubs

**Backend — dead code removal:** - Remove CanonicalValue::Bool variant and its unreachable match arm - Remove SharedCleanupAuditWriter type alias (unused Arc import with it) - Remove Page struct from plan_repo (unused, no re-export) - Remove expand_predicate from CleanupPlanRepository trait and impl (superseded by PredicateExpander) - Remove SubscriptionRepository::list_by_account from trait and all impls (never called by PlanBuilder) - Trim SubscriptionRecord to {method} only (sender/account_id/message_count were write-only) - Remove BuildError::Invalid variant (no code path constructs it) - Remove user_id from ListOpsQuery/SampleQuery (was silently ignored) - Remove stale #[allow(dead_code)] suppressions throughout - Restore #[cfg(test)] scoped imports for MockEmailProviderFactory/NoopCleanupAuditWriter - Remove #[allow(unused_imports)] blanket on orchestrator/mod.rs; keep only used re-exports - Add JobState::from_str_opt (missing counterpart to as_str)

    **Backend — stub implementations:**
    - SqlxEmailRepository: list_by_account, list_by_cluster (json_each on topic_clusters.email_ids), count_by_account
    - SqlxSubscriptionRepository::find_by_sender: queries emails table for list_unsubscribe* headers
    - SqlxClusterRepository::emails: json_each join on topic_clusters
    - SqlxAccountStateProvider::etag: joins connected_accounts + sync_state for Gmail history_id
    - SqlxRuleEvaluator::evaluate_scope: loads rules from DB + emails, calls rule_processor::evaluate_rules
    - SqliteCleanupApplyJobRepo::load + list_by_plan: proper SQL reads with map_job_row helper
    - plan_repo::sample_operations: reads sample_ids_json column from predicate rows
    - plan_repo::list_operations: filters pushed to SQL instead of in-memory only
    - build_plan_builder: async; pre-loads provider map from connected_accounts
    - reverse_for: populates ReverseOp for archive, label-add, soft-delete, and unsubscribe

    **Backend — route conflict fix:**
    - Merge /apply/{plan_id} POST and /apply/{job_id} GET into single route to eliminate axum startup panic

    **Frontend — userId derivation fix:**
    - Add useCurrentUserId hook: fetches connected accounts, returns first active account emailAddress
    - Replace static getCurrentUserId() (read localStorage 'user_id' which was never written) with reactive hook
    - Cleanup History now shows plans instead of "Sign in" prompt when already logged in

- Replace placeholder topic samples with real representative emails and add tooltips

- Step3Topics: render cluster.representativeEmails subjects instead of hardcoded
  "Example email 1/2/3" placeholders; show graceful empty-state when none present
  - ClusterVisualization: add title tooltip to truncated subject and sender lines so
    full text is visible on hover in Command Center Topic Clusters panel
  - Step3Topics: add title tooltip on each truncated subject li for the same reason
  - backend/ingestion.rs: treat phase=complete as inactive in the polling endpoint so
    InboxCleaner's fast-complete path advances the wizard correctly without hanging
  - useIngestionProgress: add FAST_COMPLETE_TIMEOUT_MS fallback to REST polling so
    already-synced accounts skip the SSE progress screen immediately
  - ingestionApi, InboxCleaner, IngestionProgress, Step2Subscriptions, SubscriptionRow,
    useInboxCleaner: supporting frontend plumbing for the above ingestion fix

- Remove dead Apply-mode code that was never wired into the pipeline

PendingAction, apply_actions, and process_email were the planned real-time
rule-application path (ADR-030 Apply mode). The cleanup pipeline only ever
used EvaluateOnly via evaluate_rules, and the test-rule endpoint no longer
calls process_email after the inbox-scan rewrite. Nothing constructs or calls
any of these three, so delete them outright.

- Guard matchCount/accuracy/isActive against undefined from backend

Backend RuleResponse does not send matchCount, accuracy, or isActive (it
sends enabled). All three were undefined when rules created via Accept were
listed, crashing toLocaleString(). Added ?? 0 / ?? enabled ?? true fallbacks
so the table renders correctly regardless of which fields the server returns.

- Provider-level action execution and match_count persistence

Root causes from Play button test: 1. run_rule only updated local SQLite; provider (Gmail/Outlook) was never
called, so the next sync overwrote the local DB change 2. match_count was not stored in the rules table, resetting to 0 on reload

    Migration 026: ADD COLUMN match_count INTEGER DEFAULT 0, last_run_at DATETIME

- Resolve stuck spinner, unsubscribe failures, and history status display

### Dependencies

- Upgrade all dependencies to latest versions

- Apply all dependabot dependency upgrades (PRs #72–#92)

Frontend (npm): - @tanstack/react-query 5.100.1 → 5.100.6 - @tanstack/react-router 1.168.23 → 1.168.25 - msw 2.13.5 → 2.14.3

    Backend (Cargo):
    - fastembed 5.13.2 → 5.13.4
    - llama-cpp-4 0.2.43 → 0.2.52
    - tokio 1.51 → 1.52.2
    - openssl 0.10.78 → 0.10.79
    - redis 1.2.0 → 1.2.1
    - mail-parser 0.11.2 → 0.11.3
    - plus 15 additional transitive crate bumps

### Documentation

- Update CHANGELOG.md for v0.1.0 [skip ci]

- Fix CHANGELOG duplication and lint errors

cliff.toml was emitting a full header (# Changelog + marker comment)
in every RELEASE_NOTES.md; the release workflow then inserted that
after the same marker in CHANGELOG.md, doubling the header on each
release. Clear cliff.toml header so release notes contain only the
version entry.

    Also fix two markdownlint errors in CHANGELOG.md:
    - MD024: remove duplicate # Changelog heading introduced by above bug
    - MD004: convert stray asterisk list item to dash

    Fix MD040 in onboarding-auth-alternatives.md (fenced code block
    missing language specifier).

- Add ADR-030 cleanup dry-run with DDD addendum, research, and implementation plan

Establishes Plan/Apply separation for the Inbox Cleaner Wizard:

    - ADR-030: cleanup dry-run with risk-scoped partial apply, hybrid materialized/predicate
      operations, and per-account drift handling.
    - DDD-008 addendum: CleanupPlanAggregate, PlanBuilder/ApplyOrchestrator/RiskClassifier
      domain services, plus DDD-007 EvaluateOnly mode.
    - Research due-diligence: competitive analysis, Gmail/Outlook/IMAP/POP3 technical limits,
      resolves the originally-deferred plan size cap, etag policy, and cross-device sync
      questions.
    - Implementation plan: phased delivery (A backend + plan API, B review screen, C apply
      + SSE + drift, D risk + telemetry; E undo and F cross-device sync deferred).
    - inception.md §10.3: supersession note pointing to ADR-030.

- Align ADR-030 / DDD-008 / plan with shipped state

Phases A–D shipped (fc56b3a, 8477c34, 98a42f3, 7a3c474) + follow-up
consolidation (a1b44a1). Updates the canonical docs to match the
lived contract.

    Plan (docs/plan/cleanup-dry-run-implementation.md)
    - Status: "Phases A–D shipped (2026-05-05). 5 of 8 follow-ups closed."
    - New §Implementation status section: commit-by-phase table; spec
      deviations adopted (storage→db, camelCase wire, engine.rs dead,
      *_json plaintext, no UnsubscribeService::execute, simulator anchor
      drift); contract additions (accountProviders, Pop3Sentinel,
      actionType, Snapshot SSE event, skipped_by_reason, 409 race-guard);
      closed follow-ups; deferred follow-ups (warrant separate ADRs);
      smaller open in-code TODOs.
    - Per-phase "Status: shipped (commit X)" markers.

    ADR-030 (docs/ADRs/ADR-030-cleanup-dry-run.md)
    - Status flipped to "Implemented (Phases A–D shipped 2026-05-05)".
    - Top-level amendment block: 10 places this ADR was superseded by
      lived contracts during delivery (storage path, camelCase wire,
      encryption interceptor non-existence, snapshot SSE variant,
      Pop3Sentinel etag, JobCounts.skipped_by_reason,
      CleanupPlan.account_providers, op events' action_type,
      refresh-while-applying 409, UnsubscribeService.execute →
      batch_unsubscribe with single-element batch).

    DDD-008 addendum (docs/DDDs/DDD-008-addendum-cleanup-planning.md)
    - Status flipped to "Implemented (2026-05-05)".
    - Amendment block listing lived domain-model additions:
      CleanupPlan.account_providers, AccountStateEtag::Pop3Sentinel +
      Pop3Invalidated reason, JobCounts.skipped_by_reason,
      SkipReason::Unacknowledged, SSE Snapshot event, op events'
      action_type field, EmailProviderFactory abstraction,
      refresh-while-applying 409, batch_unsubscribe-as-execute.

    markdownlint-cli2 clean; prettier formatted.

- Replace stale ADR/DDD tables with directory links

Individual file tables for Architecture Decisions and Domain Model
were incomplete (20+ ADRs and 10 DDDs existed, only 10 and 6 were
linked). Replace with GitHub directory links that stay current as
new documents are added.

### Features

- Phase A — backend domain + plan API

Implements ADR-030 / DDD-008 addendum Phase A: the Cleanup Planning
subdomain that materialises wizard selections into an immutable,
reviewable CleanupPlan before any provider mutation.

    Domain (backend/src/cleanup/domain/):
    - operation.rs: PlanAction, PlanSource, PlannedOperation
      (Materialized|Predicate), RiskLevel, AccountStateEtag, RiskMax,
      OperationStatus + SkipReason (Phase C ack-skip semantics),
      PlanStatus. All wire DTOs use #[serde(rename_all = "camelCase")].
    - plan.rs: CleanupPlan aggregate root, WizardSelections, PlanTotals,
      RiskRollup. canonical_plan_hash with documented profile (sorted
      keys, integer ms timestamps, sorted email-id vecs, NFC strings,
      null-not-elided) and deterministic test across reordered ops.
    - classifier.rs: pure RiskClassifier covering ADR-030 §6 (POP3
      anything → High, permanent-delete → High, web-link unsubscribe
      → High) plus bulk-threshold escalation.
    - builder.rs: PlanBuilder composes Subscriptions + Cluster actions
      + Rule selections (EvaluateOnly) + ArchiveStrategy. Dedup with
      cluster-wins-over-rule + TargetConflict warning. 30m TTL.
      Mock-provider mutation-free unit test passes; 50k-row #[ignore]
      perf test measures P95 = 71 ms (target ≤ 800 ms).
    - ports.rs: hexagonal trait surface (EmailRepository,
      SubscriptionRepository, ClusterRepository, AccountStateProvider,
      RuleEvaluator). PlanBuilder depends only on ports — never on
      providers — enforced at type-signature level.

    Repository (backend/src/cleanup/repository/):
    - plan_repo.rs: CleanupPlanRepository + SqliteCleanupPlanRepo.
      Transactional save (envelope + etags + ops), load by user_id,
      list_by_user (status + paged), list_operations (cursor by seq,
      in-memory risk/action/account filter), cancel, expire_due,
      purge_older_than. Round-trip + pagination + status-transition
      integration tests against in-memory SQLite.
    - job_repo.rs: CleanupApplyJobRepository (Phase C will drive).
    - adapters.rs: SQLx adapters for the four ports — Phase A returns
      empty/None to keep wiring compilable; Phase B/C will read real
      emails / topic_clusters / subscription tables.

    API (backend/src/cleanup/api/):
    - plan.rs: full route surface mounted at /api/v1/cleanup. Real
      handlers wired to PlanBuilder + SqliteCleanupPlanRepo via AppState.
      POST /plan returns 201 + CreatePlanResponse. GET /plan/:id,
      DELETE /plan/:id, GET /plan/:id/operations (paged), GET /sample,
      POST /refresh (account-scoped), GET /plans (history).

    Rules (backend/src/rules/):
    - types.rs: RuleExecutionMode { Apply, EvaluateOnly }, RuleEvaluation,
      EvaluationScope, RuleMatchBasis.
    - rule_processor.rs: evaluate_rules() honouring EvaluateOnly contract
      — same matcher logic, no command emission, deterministic
      head-5/tail-5/middle-stratified-10 sampling capped at 20.

- Phase B — frontend review screen

Implements ADR-030 Phase B: Step 4.5 Review & Confirm screen rendering
a Phase A plan with per-account/per-source diff groups, per-row High
and per-group Medium acknowledgers, three-tier Apply buttons (Low /
Low+Medium / All). Apply itself is a Phase-C placeholder.

    Foundation
    - frontend/packages/types/src/cleanup.ts — TS surface mirroring backend
      serde camelCase wire (PlanStatus/SkipReason/PredicateStatus values
      are camelCase; AccountStateEtag `kind` is camelCase).
    - frontend/packages/api/src/cleanupApi.ts — ky wrappers for the 7
      cleanup endpoints. userId via query param (TODO(phase-d) for auth-
      derived userId).
    - hooks/{usePlanOperations,usePlanSamples}.ts — react-query infinite-
      pagination + lazy sample cache.
    - useInboxCleaner.ts extended: buildPlan / refreshAccount /
      cancelCurrentPlan / loadPlan.

    Components (frontend/apps/web/src/features/inbox-cleaner/review/)
    - CleanupReview.tsx (orchestrator), AccountSummaryTile.tsx,
      PlanDiffGroup.tsx (virtualizes via @tanstack/react-virtual when
      rows >= 50), PlanDiffRow.tsx, RiskLegend.tsx, RiskAcknowledger.tsx
      (canonical ack-state: Set<number> high seqs + Set<string> medium
      group keys), ApplyButtons.tsx (set-difference gating: Low always
      enabled if ≥1 Low row; Low+Medium iff every Medium group acked;
      All iff every High seq AND every Medium group acked),
      PlanWarningsBanner.tsx, SampleEmailPeek.tsx,
      RefreshAccountAffordance.tsx, groupKey.ts (shared helpers).

    Wizard integration
    - InboxCleaner.tsx routes "Continue to Review" → buildPlan() → new
      "review" wizard step. Existing setInterval simulator at
      handleExecuteCleanup is kept defined but unreferenced
      (ts-expect-error) until Phase C removes it.

    Storybook (Phase B DoD bullet 4)
    - review/CleanupReview.stories.tsx — 5 scenarios: EmptyPlan,
      LowOnlyPlan, MixedRiskPlan, LargePlanWithWarnings,
      ConflictWarnings. QueryClientProvider decorator with setQueryData
      avoids network in stories.

    Test summary
    - pnpm tsc --noEmit → clean
    - pnpm lint → clean
    - pnpm build → clean (InboxCleaner chunk 71.8 KB)
    - backend cargo test cleanup:: → 35/35 (no regression)

    Phase B DoD
    - [x] /cleanup/review/:planId renders against Phase A backend
    - [x] Apply buttons gating works (set-difference on acks)
    - [~] Lighthouse a11y ≥ 95: aria-disabled, aria-label, semantic
          markup in place; not yet measured against a running dev server
    - [x] Storybook: 5 scenarios shipping
    - [~] Playwright e2e: deferred (Phase C will share a seeded fixture)

    Spec deviations / TODOs
    - @emailibrium/types collisions resolved via Cleanup* prefixes
      (ArchiveStrategy, ClusterAction, Provider, FolderOrLabel, ErrorCode).
    - AccountSummaryTile defaults provider='gmail' (envelope lacks per-
      account provider). TODO(phase-d).
    - SubscriptionInsight lacks accountId; passed as ''. TODO(phase-d).
    - Wizard local action vocab translated to cleanup vocab in
      collectSelections; null rows dropped. TODO(phase-d).

    Phase C (apply + SSE + drift handling, removes setInterval simulator)
    not started.

- Phase C — apply + SSE + drift handling

ApplyOrchestrator (backend/src/cleanup/orchestrator/): apply.rs,
sse.rs, drift.rs, expander.rs, account_worker.rs, mod.rs.
ApplyEvent SSE union (10 variants) all camelCase via serde derive.
Cancel via tokio_util::CancellationToken; pending → Skipped{User
Cancelled}; cancel = continue forward, NOT retry. AccountWorker has
account-scoped Semaphore + per-iteration sleep (write-side proactive
throttle, non-duplicative with provider's reactive
ProviderError::RateLimited). DriftDetector classifies per ADR-030 §8.
EventEmitter throttles Progress to every 250ms or every 50 ops. First
event on every SSE subscription is `snapshot` from a fresh DB read.
KeepAlive 15s.

    Apply API (backend/src/cleanup/api/apply.rs): POST /apply/:planId,
    GET /apply/:jobId/stream (axum Sse), POST /apply/:jobId/cancel,
    GET /apply/:jobId. POST /plan/:id/refresh returns 409
    { error: "apply_in_progress" } when a Running job exists.

    Phase A schema additions (additive)
    - JobCounts.skipped_by_reason: BTreeMap<SkipReason, u64>.
    - SkipReason now derives Hash/Ord/PartialOrd; Provider derives Hash.

- Phase D — risk + telemetry + history

Implements ADR-030 Phase D: cleanup-scoped audit log, structured
telemetry events, plan-history view, read-only review mode, plain-
English explainer polish, POP3 typed-confirmation gate, action-
specific risk-acknowledgement copy.

    Backend
    - migrations/025_cleanup_audit_log.sql: append-only with
      UNIQUE (plan_id, job_id, seq, outcome). NO email_id, body,
      folder paths, sender, rule body, or sample ids — enforced by
      audit_excludes_email_content test (pragma_table_info
      introspection). ADR-030 §Security: "no plan content is logged".
    - cleanup/audit.rs: CleanupAuditWriter trait + Sqlite impl
      (INSERT OR IGNORE for idempotency); list_for_plan / list_for_user
      for GDPR right-to-explanation.
    - cleanup/telemetry.rs: CleanupTelemetryEvent enum (5 variants);
      TelemetryEmitter writes via tracing::info! to "cleanup.telemetry"
      target (existing log pipeline; no new SDK). user_id and
      account_id are 8-byte truncated blake3 hashes per ADR-017.
    - cleanup/api/telemetry.rs: POST /api/v1/cleanup/telemetry —
      accepts only CleanupPlanReviewed; raw userId hashed server-side.
    - account_worker.rs: writes audit row after every
      update_operation_status. Audit failures tracing::error! but never
      abort apply.
    - apply.rs: emits CleanupApplyStarted / CleanupApplyFinished;
      ApplyOrchestrator gains builder-style with_audit / with_telemetry.
    - api/plan.rs: emits CleanupPlanBuilt (with build_duration_ms) +
      CleanupPlanRefreshed.
    - AppState: cleanup_audit_writer + cleanup_telemetry fields.

    Frontend
    - features/inbox-cleaner/history/CleanupHistory.tsx (NEW): plan
      history list view; status pills, totals, risk rollup; click →
      detail page. Empty state with CTA.
    - features/inbox-cleaner/history/CleanupHistoryDetail.tsx (NEW):
      read-only wrapper around CleanupReview; surfaces audit panel
      from listPlanAudit (graceful degradation).
    - features/inbox-cleaner/history/CleanupHistory.stories.tsx (NEW):
      Empty + WithPlans Storybook scenarios.
    - review/CleanupReview.tsx: readOnly prop hides Apply +
      RiskAcknowledger + RefreshAccountAffordance, shows role="status"
      banner, "Back to history" cancel.
    - review/PlanDiffRow.tsx: wireframe-style explainer; onSampleViewed
      callback for telemetry.
    - review/PlanDiffGroup + SampleEmailPeek: readOnly + telemetry
      callbacks.
    - review/ApplyButtons.tsx: POP3 typed-confirmation gate — High-tier
      requires typing "DELETE" when any account is POP3.
    - review/RiskAcknowledger.tsx: action-specific ack copy (delete /
      unsubscribe / web-link).
    - hooks/useCleanupTelemetry.ts (NEW): tracks expandedGroups +
      samplesViewed; emits cleanup_plan_reviewed on unmount or apply.
      Best-effort — errors swallowed.
    - cleanupApi.ts: emitCleanupReviewed + listPlanAudit + types.
    - Router.tsx: /cleanup/history + /cleanup/history/$planId.
    - Layout.tsx: "Cleanup History" main-nav entry.

    Test summary
    - cargo test cleanup:: → 67/67 + 1 ignored perf (+14 new tests for
      audit + telemetry + apply audit-write integration)
    - cargo clippy (pre-commit gate config) → clean
    - pnpm tsc --noEmit + pnpm lint (web) → clean

    Phase D DoD
    - [x] Review screen wireframe match (5 Storybook scenarios + read-
          only mode + action-specific ack copy + plain-English explainer)
    - [x] /cleanup/history list view ships, reachable from main nav
    - [x] All 5 telemetry events fire with documented payloads
    - [x] Audit-log entries written for multi-account apply tests
    - [x] High-risk ack copy reviewed
    - [x] POP3 typed-confirmation gate
    - [x] No plan content in audit/telemetry payloads

    Spec deviations / open follow-ups
    - userId via localStorage('user_id'); single migration point in
      getCurrentUserId() when a shared user store ships.
    - POP3 detection via accountStateEtags[id].kind === 'none' (plan
      envelope still lacks per-account provider — Phase B TODO).
    - cleanup_plan_reviewed time-on-review is best-effort heuristic.
    - Phase E (Undo via reverse_op) and Phase F (cross-device sync)
      remain explicitly deferred.

    Closes Phase A→D of ADR-030.

- Close phase A-D follow-ups (5 of 8)

Closes 5 of the 8 open follow-ups from Phases A-D. The remaining 3
are infrastructure-scoped and warrant separate ADRs (noted below).

    Backend
    - cleanup/orchestrator/factory.rs (NEW): EmailProviderFactory trait
      + OAuthEmailProviderFactory (Gmail/Outlook fully wired via existing
      api/provider_helpers.rs pattern; per-job in-memory provider cache)
      + MockEmailProviderFactory for tests. IMAP/POP3 yield
      FactoryError::UnsupportedKind → OpFailed with "provider_unsupported"
      error_code (clear surfacing rather than silent skip; their
      user-supplied host/credentials aren't threaded into orchestrator
      yet — small follow-up).
    - account_worker.rs: dispatch path now calls real provider methods
      via the factory. PlanAction → provider call mapping for archive,
      move, addLabel, delete, markRead, star, unsubscribe.
    - account_worker.rs: in-worker predicate expansion via
      PredicateExpander::expand_page; emits PredicateExpanded
      { predicate_seq, produced_rows } per page; status transitions
      Pending → Expanding → Expanded (or Failed). Children persisted
      with seq > max(plan.seq) and processed on next apply iteration
      (matches the partial-apply contract).
    - repository/plan_repo.rs: new methods append_operations, max_seq,
      update_predicate_status to support in-worker expansion. Also
      round-trips account_providers inline in totals_json (no schema
      migration needed).
    - domain/operation.rs: AccountStateEtag::Pop3Sentinel { last_uidl }
      variant added — POP3 has no incremental delta; any UID-LIST diff
      invalidates per ADR-030 §8. kind_str() returns "pop3_sentinel".
    - domain/plan.rs: CleanupPlan.account_providers: BTreeMap<String,
      Provider> populated by builder; included in canonical_plan_hash so
      drift detection covers re-auth / provider-type changes; round-trip
      test plan_envelope_carries_per_account_provider.
    - orchestrator/drift.rs: Pop3 hard-drift mapping; restored
      HardDriftReason::Pop3Invalidated.
    - orchestrator/sse.rs: OpApplied/OpFailed/OpSkipped events gain
      actionType: String (camelCase serde tag of the row's PlanAction);
      computed via plan_action_type_str(&PlanAction). Existing tests
      updated to assert the new field.
    - main.rs: AppState construction now builds Gmail/Outlook configs
      from OAuth config and constructs OAuthEmailProviderFactory; wired
      via ApplyOrchestrator::with_provider_factory. Empty HashMap arg
      removed from new(); existing call sites unchanged via builder
      defaults.

    Frontend
    - packages/types/src/cleanup.ts: CleanupPlan.accountProviders?:
      Record<string, CleanupProvider> (optional for backwards-compat
      with cached Phase-D plans); AccountStateEtag union extended with
      { kind: 'pop3Sentinel'; lastUidl: string }; ApplyEvent's three op
      variants gain actionType: string.
    - review/ApplyButtons.tsx: POP3 detection now reads
      plan.accountProviders[accountId] === 'pop3'. The kind === 'none'
      hack remains only as a fallback for cached plans, with a comment
      documenting the migration.
    - review/CleanupReview.tsx: detectProvider() reads
      plan.accountProviders first; gmail fallback. Plumbs
      accountProviders through to ApplyButtons. Drops the per-action
      workaround comment.
    - hooks/useCleanupApply.ts: reducer now maintains a perAction:
      Record<string, { applied; failed; skipped }> map, lazy-init
      buckets on each op event using ev.actionType. snapshot resets
      perAction; progress preserves it.
    - CleanupProgress.tsx: PerActionCounts.pending is optional
      (SSE-driven counters don't track pending).

    Test summary
    - cargo test cleanup:: → 72/72 + 1 ignored perf passing (was 67
      pre-follow-ups)
    - cargo clippy -p emailibrium --all-targets (pre-commit gate
      config) → clean
    - pnpm tsc --noEmit + pnpm lint (web) → clean

    Closed follow-ups
    - [x] OAuth-token-aware EmailProvider dispatch in account_worker
    - [x] Worker calls PredicateExpander::expand_page + emits
          PredicateExpanded
    - [x] POP3 hard-drift: AccountStateEtag::Pop3Sentinel variant added
    - [x] Per-account provider info in plan envelope
    - [x] Frontend perAction precision via SSE actionType field

    Deferred (infrastructure refactors outside cleanup scope; recommend
    separate ADRs)
    - Per-row precondition re-read against emails.labels needs schema
      work to track folder + per-action invariant rules; touches sync
      subsystem more than cleanup. Phase E candidate.
    - Encryption-at-rest interceptor for *_json columns is workspace-
      wide infra (every encrypted table needs the wrapper); should be
      a db/encryption.rs middleware module. ADR follow-up.
    - Shared current-user store: every feature today reads userId from
      localStorage. getCurrentUserId() in cleanup-history is the single
      migration point when an auth context lands. Frontend infra task.

    Smaller follow-ups still open (in-code TODOs)
    - IMAP/POP3 EmailProviderFactory support: providers' host/credentials
      aren't threaded into orchestrator. Currently surfaces as clear
      OpFailed with provider_unsupported error_code.
    - cleanup_plan_reviewed time-on-review is best-effort heuristic
      (component-unmount-after-2s). Phase D doc-only.

- Implement AI suggestions pagination, fix test-rule endpoint, and red-error styling

## AI Suggestions (GET /api/v1/rules/suggestions)

    - Added offset / limit query params for batch pagination
    - Removed hardcoded LIMIT 15 / take-5 logic; now fetches all qualifying
      senders and applies Rust-side skip/take so covered-sender filtering never
      skews the offset
    - suggestions_page_size and suggestions_min_email_count driven by new
      rules: section in config/app.yaml and yaml_config::RulesConfig

    ## Build with AI button
    - First click switches to AI Suggestions tab (as before)
    - Subsequent clicks while on the tab append the next batch of suggestions
    - AISuggestions accumulates batches internally via batchIndex prop + offsetRef
    - Loading-more spinner shown during batch fetch

    ## Test Rule endpoint (POST /api/v1/rules/test)
    - Replaced required email: TestEmail body with optional raw-JSON conditions
      parsed via json_parser (accepts flat {field,operator,value} without type tag)
    - Scans full active inbox (no LIMIT 500 cap) and returns {matchCount,
      sampleMatches} matching the frontend RuleTestResult type
    - Fixes 124 vs 40 discrepancy: suggestions and test-rule now count the same
      unbounded email corpus

    ## json_parser operator aliases
    - Added starts-with, ends-with, matches-regex so frontend dropdown values
      round-trip correctly through the backend parser

    ## RuleEditor error styling
    - testResult state changed to {message, isError} so failed tests render
      with a red banner instead of green

- Run Rule, inbound auto-apply, and AI Suggestions UX fixes

## rules/executor.rs (new shared module)

    - apply_rule_action: idempotent DB write for each RuleAction type
      (Archive→folder=ARCHIVE, MarkRead, MarkImportant, Delete, AddLabel,
      RemoveLabel; Forward is a documented no-op pending mail service)
    - apply_rules_to_email: evaluate a rule list against one email and apply
      all matching actions; returns action count

    ## POST /rules/:id/run (manual retroactive run)
    - Scans the full active inbox (unbounded) for the named rule
    - Delegates to executor.apply_rule_action so manual and automatic paths
      are always identical
    - Returns { matchCount, executedCount, sampleMatches }

    ## Inbound auto-apply on sync
    - api/ingestion.rs: load enabled rules once per sync invocation (full and
      incremental paths); call executor.apply_rules_to_email after each
      successful upsert so new emails are processed as they arrive
    - Full sync: rules applied per-message inside the page loop
    - Incremental sync: rules applied after each get_message fetch

    ## Active Rules table (frontend)
    - Play button per row with per-row loading spinner (Fragment wrapper for
      sibling result row)
    - Inline result shows "Applied to N actions across M emails" after run

    ## AI Suggestions UX fixes
    - Accept: per-card loading spinner; card removed from list on success
    - Dismiss: wired up — removes the card immediately
    - Both use dismissedIds Set; batch offsetRef unchanged

- Add draggable resize handles for sidebar and email list panels

Adds a ResizeDivider component between the sidebar/email-list and
email-list/thread-view panel boundaries, allowing users to drag and
reveal truncated category names or more email preview text.

- Archive tracking, permanent delete, and provider sync
...
**Backend — dead code removal:**
    - Remove CanonicalValue::Bool variant and its unreachable match arm
    - Remove SharedCleanupAuditWriter type alias (unused Arc import with it)
    - Remove Page struct from plan_repo (unused, no re-export)
    - Remove expand_predicate from CleanupPlanRepository trait and impl (superseded by PredicateExpander)
    - Remove SubscriptionRepository::list_by_account from trait and all impls (never called by PlanBuilder)
    - Trim SubscriptionRecord to {method} only (sender/account_id/message_count were write-only)
    - Remove BuildError::Invalid variant (no code path constructs it)
    - Remove user_id from ListOpsQuery/SampleQuery (was silently ignored)
    - Remove stale #[allow(dead_code)] suppressions throughout
    - Restore #[cfg(test)] scoped imports for MockEmailProviderFactory/NoopCleanupAuditWriter
    - Remove #[allow(unused_imports)] blanket on orchestrator/mod.rs; keep only used re-exports
    - Add JobState::from_str_opt (missing counterpart to as_str)
...
Root causes from Play button test:
    1. run_rule only updated local SQLite; provider (Gmail/Outlook) was never
       called, so the next sync overwrote the local DB change
    2. match_count was not stored in the rules table, resetting to 0 on reload
...
Frontend (npm):
    ...
    - @tanstack/react-router 1.168.23 → 1.168.25
    - msw 2.13.5 → 2.14.3
