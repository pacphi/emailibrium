...
--
-- Dialect notes (ADR-033): SQLite's datetime-affinity type -> TIMESTAMPTZ, and SQLite's
-- now-timestamp default -> now(), for sync_queue/sync_conflicts. processing_checkpoints.
-- updated_at stays TEXT (matching its SQLite type) with the now-timestamp default ported
-- to an expression producing the identical 'YYYY-MM-DD HH:MI:SS' UTC string shape.
