...
--
-- Dialect notes (ADR-033): id is SQLite's auto-increment integer primary key ->
-- GENERATED ALWAYS AS IDENTITY; plan_id/job_id/user_id/account_id are SQLite's
-- binary-blob column -> BYTEA; timestamp (unix millis) is INTEGER -> BIGINT (int4
-- overflows current epoch-millis values; SQLite's INTEGER is dynamically 8-byte so this
-- was never visible there).

CREATE TABLE IF NOT EXISTS cleanup_audit_log (
    id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    timestamp BIGINT NOT NULL,                   -- unix millis
    plan_id BYTEA NOT NULL,                      -- UUID v7 from cleanup_plans.id
    job_id BYTEA NOT NULL,                       -- UUID v7 from cleanup_apply_jobs.job_id
