...
-- existing `encrypted_access_token` BYTEA (reusing the OAuth token column and the
-- same AES-256-GCM path); these columns hold the non-secret connection details.
--
-- (SQLite only supports one ADD COLUMN per ALTER TABLE, hence one statement per line
-- below; Postgres allows combining them, but keeping this identical to the sqlite/
-- migration makes the two directories easy to diff side by side.) All columns are
-- nullable (populated only for provider = 'imap').
