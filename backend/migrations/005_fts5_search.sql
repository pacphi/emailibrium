...
-- FTS5 external-content table.  Column names MUST match the source table
-- (`emails`) so that `content-sync` rebuild reads the correct columns.
-- The `emails` table uses `id` (not `email_id`), so the FTS5 column is `id`.
