...
pub fn hdbscan(data: &[Vec<f32>], config: &HdbscanConfig) -> HdbscanResult {
    ...
    for edge in &mst {
        ...
        if pt_lambda[edge.a] == 0.0 {
            pt_lambda[edge.a] = lambda;
        }
        if pt_lambda[edge.b] == 0.0 {
            pt_lambda[edge.b] = lambda;
        }
    }
    ...
    for edge in &mst {
        ...
        if sa >= mcs && sb >= mcs {
            cc2[big] = hier
                .iter()
                .position(|h| (h.birth_lambda - lambda).abs() < 1e-10 && h.size == ns);
        } else if sa >= mcs || sb >= mcs {
    ...
    for node in hier.iter_mut() {
        let death = if node.death_lambda == f32::MAX {
            node.birth_lambda + 0.01
        } else {
            node.death_lambda
        };
...
mod tests {
    ...
    fn test_clear_cluster_structure() {
        ...
        for i in 0..6 {
            data.push(point(1.0, 0.05 * i as f32));
        }
        for i in 0..6 {
            data.push(point(0.05 * i as f32, 1.0));
        }
        ...
        if r.n_clusters >= 2 {
            ...
            let b = r.labels[6..]
                ...
                .unwrap_or(-1);
            if a >= 0 && b >= 0 {
                assert_ne!(a, b, "Clusters should have different labels");
            }
        }
    }
    ...
    fn test_noise_detection() {
        let mut data = Vec::new();
        for i in 0..6 {
            data.push(point(1.0, 0.02 * i as f32));
        }
    ...
    fn test_min_cluster_size_parameter() {
        ...
        let r = hdbscan(&data, &cfg);
        assert!(
            r.labels.iter().all(|&l| l == -1),
            "All noise when min_cluster_size > n"
        );
        assert_eq!(r.n_clusters, 0);
    }
    ...
    fn test_confidence_scores_valid() {
        ...
        for (i, &c) in r.confidences.iter().enumerate() {
            ...
            if r.labels[i] == -1 {
                assert_eq!(c, 0.0, "Noise point {i} should have 0 confidence");
            }
        }
    }
